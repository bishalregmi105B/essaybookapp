...
myframe=LabelFrame(root,text='Essays List Here',bd=1,font=('Arial',14),relief=GROOVE,width=400,height=700,bg='white')
...
text = ScrolledText(root, state='normal',height=700, width=1000, pady=2,font=('Arial',14), padx=3, undo=True)
...
def data():
    for i in range(1):
       ...
       Button(frame,text="covid19",bd=0,font=('Arial',16),command= covid19).grid(row=53,column=2)
...
data()
#######################################Functions########
develope=open(path+'developer.ash', 'r',encoding='utf-8')
developer=develope.read()
...
def othersoftware():
    url = "http://ashlya.com.np"
    webbrowser.open(url,new=new)
def developerinfo():
    text.configure(state='normal')
    text.delete(0.0,END)
    text.insert(0.0, developer)
    text.configure(state='disable')
#####################################End functions ########################
menubar = Menu(root)
filemenu = Menu(menubar)
filemenu.add_command(label="Help", command=helpe)
filemenu.add_command(label="Get our other softwares ", command=othersoftware)
filemenu.add_command(label="Developer info", command=developerinfo)
